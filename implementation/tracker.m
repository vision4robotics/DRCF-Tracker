...
% Compute the spatial regularization windows 
...
% Construct the Gaussian regularization function
% s1
Lambda = 0.7*1e-10;
Rwindow_DR_s1 = cell(numel(num_feature_blocks), 1);
for i = 1:num_feature_blocks
    sz = filter_sz_cell{i};
    output_sigma = sqrt(prod(floor(base_target_sz/feature_cell_sz(i)))) * 0.248; %Regularization_sigma_factor 0.248
    rg           = circshift(-floor((sz(1)-1)/2):ceil((sz(1)-1)/2), [0 -floor((sz(1)-1)/2)]);
    cg           = circshift(-floor((sz(2)-1)/2):ceil((sz(2)-1)/2), [0 -floor((sz(2)-1)/2)]);
    [rs, cs]     = ndgrid(rg,cg);
    Rwindow_DR_s1{i}    = fftshift( exp(-0.5 * (((rs.^2 + cs.^2) / output_sigma^2))) );
end

% s2 
Rwindow_DR_s2 = cell(numel(num_feature_blocks), 1);
for i = 1:num_feature_blocks
%     sz = filter_sz_cell{i};
    Rwindow_DR_s2{i} = 1e5./ (0.1 + Rwindow_DR_s1{i});
end
...
while true
    ...
    if seq.frame > 1
        ...
        %check for unreasonable pos
        if pos(1)<= -250 || pos(1) >=imgH+250
            %target lost and relocate tracking position and scale
            pos=[imgH,imgW]/2;
            target_sz=init_target_sz;
            reinit = 1;
        elseif pos(2)<= -250 || pos(2)>=imgW+250
            pos=[imgH,imgW]/2;
            target_sz=init_target_sz;
            reinit = 1;
        end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Extarct Saliency Map
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    %SRDCF+DR Reverse Gaussian 
%     reg_window_input_s2{1} =  Rwindow_DR_s2{1} ;
%     reg_window_input_s1{1} =  Rwindow_DR_s2{1} ;
    ...
    if min(tracking_result.target_size) <= do_sa_target_size
        ...
        reg_window_input_s2{1} = reg_window{1} ;
        reg_window_input_s1{1} = params.reg_window_min ./ reg_window{1} ;
        reg_window_input_s2{1} = reg_window_input_s2{1}.*sqrt(Lambda);
        
        %Show the regularization map S1 and S2
        if  params.visualization_sa
            figure(20)
            surf(reg_window_input_s2{1});
            
            figure(10)
            surf(reg_window_input_s1{1})
        end
        ...
        try
            ...
            reg_window_input_s2{1} =  params.reg_window_max *ones(filter_sz);
            
            reg_window_input_s2{1}( m_input(:) <=1 & reg_window{1}(:) <=1 ) = params.reg_window_min;
        catch
            reg_window_input_s2{1} = reg_window{1};
        end
        
        %creat and standardize S1
        try
            reg_window_input_s1{1} = ones(size(reg_window_input_s2{1}));
            reg_window_input_s1{1}( reg_window_input_s2{1}(:) >=1 ) = params.reg_window_min;
            reg_window_input_s1{1}( reg_window_input_s2{1}(:) <1 )  = params.reg_window_max;
            reg_window_input_s1{1} = reg_window_input_s1{1} ./ params.reg_window_max;
        catch
            reg_window_input_s1{1} = ones(size(reg_window_input_s2{1}));
        end
        
        %This is the final strive
        reg_window_input_s2{1} = reg_window_input_s2{1}.*sqrt(Lambda);
...
%     reg_window_input_s1{1} = ones(size(reg_window_input_s2{1}));
    ...
    for k = 1: numel(xlf)
        ...
        if params.use_gpu
            ...
            w_f = gpuArray(w_f);
            h_f = gpuArray(h_f);
            reg_window_input_s1{k} = gpuArray(reg_window_input_s1{k});
            reg_window_input_s2{k} = gpuArray(reg_window_input_s2{k});
            yf{k} = gpuArray(yf{k});
        end
        ...
        while (iter <= admm_max_iterations)
            ...
            s_odot_w_f = fft2( ifft2(w_f) .* reg_window_input_s1{k} );
            
            % subproblem g
            Pool__extract_info = S_xx + rho;
            Ssodotwx_f = sum(conj(model_xf) .* s_odot_w_f , 3);
            Shx_f = sum(conj(model_xf) .* h_f, 3);
            
            g_f = ((1 / rho  * bsxfun(@times,  yf{k}, model_xf)) -  h_f + s_odot_w_f ) - ...
                bsxfun(@rdivide,( 1/ rho * bsxfun(@times, model_xf, (S_xx .*  yf{k})) - ...
                (bsxfun(@times, model_xf, Shx_f)) + (bsxfun(@times, model_xf, Ssodotwx_f))), Pool__extract_info);
            
            %   subproblem w
            w_f = fft2(...
                ...
                argmin_w(reg_window_input_s1{k},reg_window_input_s2{k},...
                rho,...
                (rho * reg_window_input_s1{k}.* real(ifft2(g_f+ h_f)) )...
                , w_f)...
                ...
                );
            
            %   update h
            h_f = h_f +  g_f - s_odot_w_f ;
        ...
        elseif reinit ==1
            ...
            cf_f{k} = (1 - params.Lrate) *  cf_f{k} + params.Lrate * ( w_f ) ;
        end
